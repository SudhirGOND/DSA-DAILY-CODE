...
//
//    }
//// this is code of the memoization
...
//        int Mincost = Integer.MAX_VALUE;
//
//        for (int k = i; k < j; k++) {  //// fromt the left most to the right most traverse
...
//        System.out.println(mincosttabu(arr));
        ...
        System.out.println(mincostoftheMatrix1d(0, n - 2, arr));
    }

    private static int mincostoftheMatrix1d(int i, int j, int[] arr) {

        //// TUIS IS THE CODE Of  THE RECURSION
        ...
        for (int k = i; k < j; k++) {
            ...
            int multicost = arr[i] * arr[j + 1] * arr[k + 1];
            int totalcost = mincostoftheMatrix1d(i, k, arr) + mincostoftheMatrix1d(k + 1, j, arr) + multicost;
            mincost = Math.min(mincost, totalcost);
        }
        return mincost;
